class Organisms {
    ...
    killOrganism = (color) => {
        for(let i = 0; i < this.#field.length; i++)
            for (let j = 0; j < this.#field.length; j++)
            {
                if(this.#field[i][j] == color) this.#field[i][j] = 'dead';
            }       
    }
    destroyTheWorld = () => {
        for(let i = 0; i < this.#field.length; i++)
...
self.onmessage = (e) => {
    ...
    else if (e.data.split(' ')[1] == 'add'){
        organisms.addOrganism(e.data.split(' ')[0]);
    }
    else if (e.data.split(' ')[1] == 'kill'){
        organisms.killOrganism(e.data.split(' ')[0]);
    }
    else if (e.data.split(' ')[1] == 'destroy'){
        organisms.destroyTheWorld();
    }
}